  ...
  window.dkExtrairCadastroClienteDocumentos = async function (apiKey, input) {
    ...
    try {
      ...
      if (err && err.name === "AbortError") {
        throw new Error(
          "Tempo esgotado (rede lenta ou fotos muito pesadas). Tente Wi‑Fi ou fotos com menos zoom."
        );
      }
      throw err;
    }
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        throw new Error("Chave OpenAI inválida ou expirada. Salve de novo em Comprovante (IA).");
      }
      if (res.status === 429) {
        throw new Error("Limite da API OpenAI atingido. Aguarde um minuto e tente de novo.");
      }
      if (res.status === 413 || /payload|too large|maximum/i.test(detail)) {
        throw new Error(
          "Imagens grandes demais para a API. Tire fotos um pouco mais de longe ou use boa luz (o app já reduz o tamanho)."
        );
      }
