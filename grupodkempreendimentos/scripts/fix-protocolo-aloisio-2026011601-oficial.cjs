/**
 * Remove protocolo errado 2026082801 (desalinhado da data início)
 * e mantém 2026011601 (Aloisio / SOU5C59 / início 16/01/2026) na nuvem oficial.
 *
 *   node grupodkempreendimentos/scripts/fix-protocolo-aloisio-2026011601-oficial.cjs
 */
...
const WRONG = "2026082801";
const RIGHT = "2026011601";
const CPF = "00175015554";
const PLACA = "SOU5C59";
const INICIO = "16/01/2026";
...
function prefer(target, source, keys) {
  for (const k of keys) {
    const src = source[k];
...
function fixPayload(payload) {
  const report = { removedWrong: false, enrichedRight: false, before: 0, after: 0, rightOk: false };
  ...
  if (wrongIdx >= 0 && rightIdx >= 0) {
    ...
    if (!String(right.nome || "").trim() && wrong.nome) right.nome = wrong.nome;
    if (!onlyDigits(right.cpf)) right.cpf = CPF;
    if (!normPlaca(right.placa)) right.placa = PLACA;
    if (!String(right.inicio || "").trim()) right.inicio = INICIO;
    right.numeroContrato = RIGHT;
    right.origemPortal = true;
    right.cadastroRetroativo = true;
    ...
    payload.dk_locacoes_cadastro[wrongIdx] = {
      ...
      inicio: String(wrong.inicio || "").trim() || INICIO,
      origemPortal: true,
  ...
  report.rightOk = Boolean(
    right &&
      ...
      String(right.inicio || "").trim() === INICIO
  );
...
async function main() {
  ...
  console.log("Verify:", {
    ...
    right: v.right
      ? {
          ...
          fim: v.right.fim,
          valorInvestimento: v.right.valorInvestimento,
          valorSemanal: v.right.valorSemanal,
        }
      : null,
  });
  ...
  console.log("OK — protocolo correto 2026011601; 2026082801 removido.");
}
