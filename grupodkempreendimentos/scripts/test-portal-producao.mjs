...
async function runSuite() {
  ...
  try {
    ...
    const finCeoPagJs = await fetch(`${BASE_URL}portal-financeiro-ceo.js?v=20260912noloop`, {
      cache: "no-store",
    }).then((r) => r.text());
    record(
    ...
    const syncSupaPtJs = await fetch(`${BASE_URL}portal-supabase-sync.js?v=20260912noloop`, {
      cache: "no-store",
    }).then((r) => r.text());
    record(
