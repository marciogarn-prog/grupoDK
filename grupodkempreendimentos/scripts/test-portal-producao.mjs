...
async function runSuite() {
  ...
  try {
    ...
    const finCeoPagJs = await fetch(`${BASE_URL}portal-financeiro-ceo.js?v=20260916parcelas`, {
      cache: "no-store",
    }).then((r) => r.text());
    record(
    ...
    record(
      "painel CEO resume planos semanais ativos",
      html.includes("finCeoKpiBoxPlanosAtivos") &&
        html.includes("DK Meu Transporte moto") &&
        finCeoPagJs.includes("resumoPlanosAtivosCeo") &&
        finCeoPagJs.includes("classificarPlanoAtivoCeo"),
      "locação, investimento e totais por plano"
    );
    record(
      "contador da parcela mostra atual e total da série",
      finCeoPagJs.includes("totalPagamentos") &&
        finCeoPagJs.includes('padStart(2, "0")}/${String(totalPagamentos).padStart(2, "0")'),
      "ex.: PAGAMENTO 01/08, 02/08 … 08/08"
    );
    record(
    ...
    record(
      "despesa grava em bloco sem reler a base",
      finCeoPagJs.includes("montarPayloadBlocoCeo") &&
        finCeoPagJs.includes("aplicarBlocoNaTela") &&
        finCeoPagJs.includes("patch: true"),
      "lançar/apagar/pagar só enviam o bloco"
    );
    record(
      "excluir despesa pergunta evento ou série",
      finCeoPagJs.includes("abrirModalExcluirDespesa") &&
        finCeoPagJs.includes("excluirDespesaSerie") &&
        html.includes("finCeoDespExcluirModal") &&
        html.includes("Excluir só este lançamento"),
      "modal pergunta se apaga um ou toda a série"
    );
    record(
      "excluir só este lançamento continua clicável",
    ...
    record(
      "submenu lançamento aluguel (avulso + relatórios 2.1/2.2/2.3/2.4/2.5)",
      htmlLancAluguel.includes("btn-lanc-aluguel-avulso") &&
        ...
        htmlLancAluguel.includes("btn-lanc-aluguel-rel-ceo") &&
        htmlLancAluguel.includes("btn-lanc-aluguel-rel-cliente") &&
        ...
        htmlLancAluguel.includes("portalRelCeoInicio") &&
        htmlLancAluguel.includes("portalRelClienteVidaBusca") &&
        ...
        htmlLancAluguel.includes("operacaoLancAluguelPaneRelCeo") &&
        htmlLancAluguel.includes("operacaoLancAluguelPaneRelCliente") &&
        htmlLancAluguel.includes("O ADMINISTRADOR CEO TEM O PODER DE APAGAR PAGAMENTOS") &&
    ...
    record(
      ...
      portalUiLancJs.includes("OPERACAO_LANC_ALUGUEL_SUB_ATIVOS") &&
        ...
        portalUiLancJs.includes('"rel-ceo"') &&
        portalUiLancJs.includes('"rel-cliente"') &&
        ...
        portalUiLancJs.includes("anexarPortalPagamentoAuditoria") &&
        portalUiLancJs.includes("collectPortalRelClienteVida") &&
        portalUiLancJs.includes('linha("Comentário", comentario)') &&
