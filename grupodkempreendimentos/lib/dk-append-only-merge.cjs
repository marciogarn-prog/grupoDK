...
function mergePortalLancamentosAluguelEmbutidos(arrays) {
  ...
  const tipoMov = (o) => {
    if (!o || typeof o !== "object") return "PAGAMENTO";
...
function mergeManutencoesRapidas(previousList, incomingList) {
  const byId = new Map();
  const add = (r) => {
...
function scoreFinanceiroRegistro(x) {
  const u = Number(x && x.updatedAt) || 0;
  if (u > 0) return u;
  return Date.parse((x && (x.cadastradoEm || x.criadoEm || x.updated_at)) || 0) || 0;
}

function pickFinanceiroCeoRegistro(ex, r) {
  const se = scoreFinanceiroRegistro(ex);
  const sr = scoreFinanceiroRegistro(r);
  if (sr > se) return { ...ex, ...r };
  if (se > sr) return { ...r, ...ex };
  if (r.ceoAutoridade && !ex.ceoAutoridade) return { ...ex, ...r };
  return { ...r, ...ex };
}

function mergeFinanceiroById(previousList, incomingList, opts) {
  const byId = new Map();
  const add = (r) => {
    if (!r || typeof r !== "object") return;
    const id = String(r.id || "").trim();
    ...
    const newer = pickFinanceiroCeoRegistro(ex, r);
    const excluidos = [
      ...new Set(
  ...
  if (opts && opts.dropDeleted) out = out.filter((x) => !x.deleted);
  return out;
}

function mergeFinanceiroCeoDespesas(previousList, incomingList) {
  return mergeFinanceiroById(previousList, incomingList);
}

function mergeFinanceiroDespesas(previousList, incomingList) {
  return mergeFinanceiroById(previousList, incomingList, { dropDeleted: true });
}

function scoreSituacaoPagCeo(x) {
  const u = Number(x && x.updatedAt) || 0;
  if (u > 0) return u;
  return Date.parse((x && x.pagoEm) || 0) || 0;
}

function mergeFinanceiroCeoSituacaoPag(previousList, incomingList) {
  ...
  const add = (r) => {
    ...
    const se = scoreSituacaoPagCeo(ex);
    const sr = scoreSituacaoPagCeo(r);
...
function neverLoseCadastroPayload(existing, incoming) {
  ...
  const pairs = [
    ...
    ["dk_manutencoes_rapidas_v1", mergeManutencoesRapidas],
    ["dk_financeiro_ceo_despesas_v1", mergeFinanceiroCeoDespesas],
    ["dk_financeiro_despesas_v1", mergeFinanceiroDespesas],
    ["dk_financeiro_ceo_fontes_v1", mergeFinanceiroById],
    ["dk_financeiro_ceo_cartoes_v1", mergeFinanceiroById],
  ...
  const sugKey = "dk_manutencao_rapida_sugestao_oleo_v1";
  const sugA = existing[sugKey] && typeof existing[sugKey] === "object" && !Array.isArray(existing[sugKey]) ? existing[sugKey] : null;
  const sugB = incoming[sugKey] && typeof incoming[sugKey] === "object" && !Array.isArray(incoming[sugKey]) ? incoming[sugKey] : null;
  if (sugA || sugB) {
    const tsA = Date.parse((sugA && sugA.updatedAt) || 0) || 0;
    const tsB = Date.parse((sugB && sugB.updatedAt) || 0) || 0;
    out[sugKey] = !sugA ? sugB : !sugB ? sugA : tsB >= tsA ? sugB : sugA;
  }
