...
function buildSummaryHtml(payload, meta) {
  ...
  return `
    ...
    <p style="color:#666;font-size:12px">${meta.footerNote || "Guarde o ficheiro em local seguro."}</p>
  `;
}
...
async function sendBackupEmail(payload, baseName, opts = {}) {
  ...
  const subject = opts.subject || `DK Backup diário — ${baseName}`;
  const html = buildSummaryHtml(payload, {
    ...
    footerNote:
      opts.footerNote ||
      (payload.source === "dk-portal-browser"
        ? "Gerado manualmente no portal (botão Gerar backup)."
        : "Gerado por cron Vercel."),
  });
