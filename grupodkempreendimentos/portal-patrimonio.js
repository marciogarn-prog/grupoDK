...
(function portalPatrimonio() {
  ...
  /** Campos que mudam numa atualização anual do CRLV (demais dados fixos do veículo). */
  ...
  const PATRIMONIO_FILA_TTL_MS = 15 * 60 * 1000;
  ...
  function sanitizeFotoCaptura(f) {
    ...
    return {
      ...
      modoEnvio:
        String(f.modoEnvio || "").trim() === PATRIMONIO_MODO_ATUALIZACAO
  ...
  function patrimonioPlacasCadastradasSet() {
    const set = new Set();
    getDocumentos().forEach((d) => {
  ...
  function filtrarArquivosPdfPorModo(files, modo) {
    ...
    return { aceitos, ignorados, semPlaca };
  }

  function mesclarCamposAtualizacaoCrlv(existente, novosCampos) {
    if (!existente || !novosCampos) return novosCampos || existente;
    const out = { ...existente };
    for (const key of PATRIMONIO_CAMPOS_ATUALIZACAO) {
      const v = String(novosCampos[key] ?? "").trim();
      if (v) out[key] = v;
    }
    if (novosCampos.placa) {
      out.placa = novosCampos.placa;
      out.placaNorm = novosCampos.placa;
    }
    return out;
  }
  ...
  function exclusaoPatrimonioEhDefinitiva(ex) {
    ...
    if (
      /prazo esgotado|imagem indisponível|esgotou.*tentativas|leitura ia|processamento interrompido|não foi possível concluir|fila órfã|reenvie o pdf/.test(
        motivo
      )
    ) {
      return true;
  ...
  async function patrimonioExpurgarFilaObsoleta() {
    ...
    for (const f of fotos) {
      ...
      if (idade >= PATRIMONIO_FILA_TTL_MS) {
        const mins = Math.max(1, Math.round(PATRIMONIO_FILA_TTL_MS / 60000));
        if (
          await excluirFotoCapturaAutomatico(
            f.id,
            `Prazo esgotado (${mins} min na fila) — reenvie o PDF se ainda precisar.`
          )
        ) {
          removidos++;
        }
  ...
  /** Após tentativa de processamento: exclui tudo o que ainda estiver pendente (processa ou exclui). */
  async function patrimonioExpurgarFilaPendenteFinal() {
    const fotos = getFotosCapturas().filter((f) => fotoEstaPendenteIa(f));
    let removidos = 0;
    for (const f of fotos) {
      const motivo = String(f.msgIa || "Não foi possível concluir a leitura IA — reenvie o PDF.").slice(
        0,
        220
      );
      if (await excluirFotoCapturaAutomatico(f.id, motivo)) removidos++;
    ...
    if (removidos > 0) {
      await patrimonioApagarImagensFilaOrfas();
  ...
  async function patrimonioApagarImagensFilaOrfas() {
    const idsAtivos = new Set(getFotosCapturas().map((f) => f.id));
    let rawStore;
    try {
    ...
    const candidatos = [
      ...(Array.isArray(rawStore.fotosCapturas) ? rawStore.fotosCapturas : []),
      ...todasExclusoesPatrimonio(rawStore.fotosCapturasExcluidas),
    ];
    for (const item of candidatos) {
      const id = String(item?.id || "").trim();
      if (!id || idsAtivos.has(id)) continue;
  ...
  async function patrimonioRetomarFilaIa(opts = {}) {
    ...
    if (
      ...
      expurgadosFinal = await patrimonioExpurgarFilaPendenteFinal();
      if (!patrimonioIaEmCurso()) {
        limpos += await patrimonioAplicarLimpezaArquivosEnviados({ expurgarTudo: true });
      }
    }
    const alterouFila = expurgados > 0 || limpos > 0 || expurgadosFinal > 0;
    if (!silencioso || alterouFila) {
      renderFotosLista();
      renderLista();
      atualizarBotaoErrosPatrimonio();
      if (!silencioso && (expurgados > 0 || expurgadosFinal > 0)) {
        setMsg(
          `${expurgados + expurgadosFinal} arquivo(s) removido(s) da fila (presos, prazo esgotado ou sem imagem).`,
          false,
          true
        );
      }
  ...
  function patrimonioLimparArquivosEnviados(storeIn, opts) {
    ...
    for (const f of ordenadas) {
      ...
      if (placaEfetiva && placasDoc.has(placaEfetiva) && String(f.modoEnvio || "") !== PATRIMONIO_MODO_ATUALIZACAO) {
        marcarExclusao(f);
        continue;
      }
  ...
  async function processarIaParaFotoCaptura(fotoId, imagens) {
    ...
    try {
      ...
      const modoEnvio = String(fotoMeta?.modoEnvio || PATRIMONIO_MODO_NOVO);
      let camposFinal = campos;
      if (modoEnvio === PATRIMONIO_MODO_ATUALIZACAO) {
        if (!existente) {
          await tratarFalhaIaFotoCaptura(
            fotoId,
            `Placa ${campos.placa} não está cadastrada — use a área «Novos».`
          );
          return;
        }
        camposFinal = mesclarCamposAtualizacaoCrlv(existente, campos);
      }
      const agora = new Date().toISOString();
      const doc = {
        ...camposFinal,
      ...
      setMsg(
        r.substituiu
          ? modoEnvio === PATRIMONIO_MODO_ATUALIZACAO
            ? `Placa ${campos.placa}: CRLV atualizado (exercício, proprietário, local/data e observações).`
  ...
  async function registrarArquivoPdf(file, imagemRenderizada, opts) {
    const modoEnvio =
      String(opts?.modoEnvio || PATRIMONIO_MODO_NOVO) === PATRIMONIO_MODO_ATUALIZACAO
        ? PATRIMONIO_MODO_ATUALIZACAO
        : PATRIMONIO_MODO_NOVO;
    const agora = new Date();
  ...
  async function processarArquivosPdf(fileList, opts) {
    ...
    if (modo === PATRIMONIO_MODO_COMPLETAR && (jaRelatorio.length || jaFila.length)) {
      ...
      const motivo =
        modo === PATRIMONIO_MODO_ATUALIZACAO
          ? "placa ainda não cadastrada (use «Novos»)"
          : "placa já cadastrada (use «Atualização» ou ignore)";
      setMsg(
        `${filtrado.ignorados.length} PDF(s) ignorado(s) — ${motivo}: ${placasIgn.slice(0, 12).join(", ")}${placasIgn.length > 12 ? "…" : ""}.`,
        false
      );
    }
    if (!files.length) {
    ...
    patrimonioPausarSyncCloud();
    await patrimonioAtivarWakeLock();
    setMsg(`A receber ${files.length} ficheiro(s) (${rotuloModo}, máx. ${PATRIMONIO_MAX_LOTE})…`, false);
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const imagem = await pdfArquivoParaImagemAlta(file);
        const foto = await registrarArquivoPdf(file, imagem, { modoEnvio: modo });
        if (!foto) {
  ...
  function abrirSeletorPdf(modo) {
    const input = obterInputPdf(modo);
    if (input) {
      input.click();
      return;
    }
  ...
  function onPatrimonioPdfInputChange(ev) {
    ...
    void processarArquivosPdf(files, { modo });
  }

  function bindPatrimonioPdfDropzone(zone, modo) {
    ...
    zone.addEventListener("drop", (ev) => {
      ...
      if (files?.length) void processarArquivosPdf(files, { modo });
    });
  }
  ...
  function bindPatrimonioPdfUpload() {
    ...
    bindPatrimonioPdfDropzone(
      document.getElementById("patrimonioPdfDropzoneNovos") || pdfDropzoneNovos,
      PATRIMONIO_MODO_NOVO
    );
    bindPatrimonioPdfDropzone(
